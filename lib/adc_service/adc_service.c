...
void adc_service_adc1_config(void){
    ...
    if (unit == ADC_UNIT_1) {
        ...
        adc1_config_channel_atten(ADC1_CHANNEL, ADC1_ATTEN);
    } else {
        adc2_config_channel_atten((adc2_channel_t)ADC1_CHANNEL, ADC1_ATTEN);
    }
...
int adc_service_adc1_read(void){
	...
	for (int i = 0; i < NO_OF_SAMPLES; i++) {
		if (unit == ADC_UNIT_1) {
			...
			int raw;
			adc2_get_raw((adc2_channel_t)ADC1_CHANNEL, width, &raw);
			adc_reading += raw;
		}
	}
